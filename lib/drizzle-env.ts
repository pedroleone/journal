import { existsSync, readFileSync } from "node:fs";
import { join } from "node:path";

type DrizzleEnv = Pick<NodeJS.ProcessEnv, "TURSO_DATABASE_URL" | "TURSO_AUTH_TOKEN">;

type ResolveDrizzleDbCredentialsOptions = {
  cwd?: string;
  env?: DrizzleEnv;
};

function parseEnvValue(rawValue: string) {
  const trimmed = rawValue.trim();

  if (!trimmed) return "";

  if (
    (trimmed.startsWith("\"") && trimmed.endsWith("\"")) ||
    (trimmed.startsWith("'") && trimmed.endsWith("'"))
  ) {
    return trimmed.slice(1, -1);
  }

  const commentIndex = trimmed.indexOf(" #");
  return commentIndex === -1 ? trimmed : trimmed.slice(0, commentIndex);
}

function readEnvFile(filePath: string) {
  if (!existsSync(filePath)) return {};

  const contents = readFileSync(filePath, "utf8");
  const values: Partial<DrizzleEnv> = {};

  for (const line of contents.split(/\r?\n/)) {
    const trimmed = line.trim();
    if (!trimmed || trimmed.startsWith("#")) continue;

    const normalized = trimmed.startsWith("export ") ? trimmed.slice(7).trim() : trimmed;
    const separatorIndex = normalized.indexOf("=");
    if (separatorIndex === -1) continue;

    const key = normalized.slice(0, separatorIndex).trim();
    const value = parseEnvValue(normalized.slice(separatorIndex + 1));

    if (key === "TURSO_DATABASE_URL" || key === "TURSO_AUTH_TOKEN") {
      values[key] = value;
    }
  }

  return values;
}

export function resolveDrizzleDbCredentials(
  options: ResolveDrizzleDbCredentialsOptions = {},
) {
  const cwd = options.cwd ?? process.cwd();
  const env = options.env ?? process.env;
  const envLocal = readEnvFile(join(cwd, ".env.local"));
  const envFile = readEnvFile(join(cwd, ".env"));

  const url =
    env.TURSO_DATABASE_URL ||
    envLocal.TURSO_DATABASE_URL ||
    envFile.TURSO_DATABASE_URL ||
    "file:local.db";

  const authToken =
    url.startsWith("file:")
      ? undefined
      : env.TURSO_AUTH_TOKEN || envLocal.TURSO_AUTH_TOKEN || envFile.TURSO_AUTH_TOKEN;

  return { url, authToken };
}
